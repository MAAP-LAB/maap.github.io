...
from itertools import repeat
from blap.model.AudioEncoder.utils import do_mixup, interpolate

from blap.model.AudioEncoder.feature_fusion import iAFF, AFF, DAF
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False, device=None):# out_feat_keys: List[str] = None):
        ...
        if not self.enable_fusion:
            # x = x.clone().detach().requires_grad_(True).to(device=device, non_blocking=True)
            ...
            raise "Feature Fusion is not Supported"
