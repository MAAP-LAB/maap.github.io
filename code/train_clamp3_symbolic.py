...
class TextMusicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text_inputs = tokenizer(text, return_tensors='pt', truncation=True)
        ...
        # load music file
        if self.mode == 'train':
            # filepaths가 문자열이면 리스트로 변환
            paths = item["filepaths"]
            if isinstance(paths, str):
                paths = [paths]
                # 실제 파일이 존재하는 첫 번째 경로 선택 (train에서도 보장!)
                for p in paths:
                    abs_path = os.path.normpath(os.path.join(self.datapath, p))
                    if os.path.isfile(abs_path):
                        filepath = abs_path  # 절대경로!
                        break
            else:
                raise FileNotFoundError(f"No valid filepath in {item['id']}: {paths}")
        else:
            # filepaths가 문자열이면 리스트로 변환
            paths = item["filepaths"]
            if isinstance(paths, str):
                paths = [paths]
            # .abc 우선, 없으면 첫 번째 존재 파일 선택
            paths = sorted(paths, key=lambda p: (not p.endswith(".abc"), p))
            for p in paths:
                abs_path = os.path.normpath(os.path.join(self.datapath, p))
                if os.path.isfile(abs_path):
                    filepath = abs_path  # 절대경로!
