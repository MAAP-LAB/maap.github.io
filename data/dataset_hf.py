...
class DataLoaderConfig:
    ...
    use_processors:List[str]= field(default_factory=lambda: ['filter_duration','map_prepare_dataset'])
    max_duration:int = 300
    min_duration:int = 200
    feature_extractor=WhisperFeatureExtractor.from_pretrained("openai/whisper-small")
    out_vis:str='./'

class DataLoaderHF(DataLoaderConfig):
    def __init__(self, args: DataLoaderConfig):
        ...
        map_fn = {
            'map_prepare_dataset':partial(self.padding,feature_extractor=self.feature_extractor),
        }
        
        self.filters = [filter_fn.get(fn) for fn in self.use_processors if fn.split('_')[0] == 'filter']
        self.maps = [map_fn.get(fn) for fn in self.use_processors if fn.split('_')[0] == 'map']
    ...
    @staticmethod
    def padding(example,feature_extractor):
...
def load_dataset_hf(args:DataLoaderConfig):
    ...
    """
    dataloader=DataLoaderHF(args)
    batch_size=dataloader.batch_size
    num_proc=dataloader.num_proc
    preprocess=dataloader.is_process

    dataset = dataloader.load()
    if preprocess:
        for preprocessor in dataloader.filters:
            dataset = dataset.filter(preprocessor,batched=True,batch_size=batch_size, num_proc=num_proc)
        
        for postprocessor in dataloader.maps:
